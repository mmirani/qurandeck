...
- Notes, favorites, highlights, a completion map, and reading time.
...
- `sealed`: AES-256-GCM ciphertext of the library JSON (preferences, bookmarks, notes, highlights, swatches, progress, display name, profile photo or icon).
- `updated_at`

QuranDeck can decrypt a library while the reader is signed in, so sync works. The table itself is not readable text. Deleting a cloud library is a request for now; there is no delete button yet.
